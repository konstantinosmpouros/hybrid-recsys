...
class ContentBasedRecommender:
    ...
    def neighbors(self, movie_id: int, k: int = 10) -> list[tuple[int, float]]:
        """Top-k most content-similar movies to `movie_id` as (movieId, cosine).

        Powers the movie-explorer ("items like this") and the "why this?"
        explanation. Capped at the precomputed `n_neighbors` pool.
        """
        if self.movie_index is None or movie_id not in self.movie_index.index:
            return []
        mids, sims = self._similar_items(movie_id)
        return [(int(m), float(s)) for m, s in zip(mids[:k], sims[:k])]

    def similarity_to(self, movie_id: int, other_ids) -> dict[int, float]:
        """Cosine similarity of `movie_id` to each id in `other_ids`.

        Used to explain a recommendation by surfacing which of the user's
        already-rated movies are most similar to the recommended one.
        """
        if self.movie_index is None or movie_id not in self.movie_index.index:
            return {}
        v = self._features_norm[self.movie_index[movie_id]]
        out = {}
        for m in other_ids:
            if m in self.movie_index.index:
                out[int(m)] = float(self._features_norm[self.movie_index[m]] @ v)
        return out
